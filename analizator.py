...
Добавлена фильтрация по спискам IP/доменов, связанных с РФ.
Генерация нескольких подписок: sub_android.txt, sub_ios.txt и по протоколам.

Последние изменения:
- Отладочная печать HTML, если канал вернул 0 прокси (первые 1000 символов).
- Сохранение пустой подписки при отсутствии конфигов (без падения).
- Улучшенный парсинг Telegram (code, pre, inline buttons).
"""
...
class TelegramWebLoader:
    ...
    - если не найдено ни одной прокси-ссылки, выводит в лог фрагмент HTML для диагностики
    """
    BASE = "https://t.me/s/"
    MAX_PAGES = 5
    ...
    async def fetch(self, channel: str) -> List[str]:
        ...
        for page in range(self.MAX_PAGES):
            ...
            try:
                async with self.session.get(url, timeout=20) as resp:
                    ...
                    if page == 0:
                        debug_html = html  # сохраняем первую страницу для отладки
                    soup = BeautifulSoup(html, 'lxml')
                    messages = soup.find_all('div', class_='tgme_widget_message_wrap')
                    if not messages:
                        logger.info(f"TG {channel}: no messages on page {page+1}, stopping pagination")
                        break
                    ...
                    for msg in messages:
                        ...
                        # 2. Код-блоки
                        for code_tag in msg.find_all(['code', 'pre']):
                            all_links.extend(extract_proxy_links(code_tag.get_text()))

                        # 3. Встроенные кнопки
                        for btn in msg.find_all('a', class_='tgme_widget_message_inline_button'):
                            href = btn.get('href')
                            if href:
                                all_links.extend(extract_proxy_links(href))
                                if any(p in href for p in ['vmess://', 'vless://', 'trojan://', 'hy2://', 'ss://', 'ssr://']):
        ...
        # Диагностический вывод, если ничего не собрано
        if not all_links and debug_html:
            logger.warning(f"TG {channel}: 0 proxies found. Dumping first 1000 chars of HTML:\n{debug_html[:1000]}")
        else:
...
class Analizator:
    ...
    async def collect(self, urls: List[str] = None, channels: List[str] = None) -> List[str]:
        ...
        raw = set()
        if urls:
            loader = URLLoader(self.session)
            for url in urls:
                links = await loader.fetch(url)
                raw.update(links)
        if channels:
            tg_loader = TelegramWebLoader(self.session)
            for ch in channels:
    ...
    async def update(self, sources: dict):
        ...
        if not new_cfgs:
            ...
            self._build_subscriptions()   # всё равно сгенерируем (возможно, пустую, но без ошибок)
            return
    ...
    def _build_subscriptions(self):
        """
        Генерирует подписки. Если хранилище пусто, создаёт файлы с одним заголовком (пустая подписка).
        """
        if not self.storage:
            logger.warning("No configs available – generating empty subscription files")
            # создаём пустую подписку, чтобы не ломать pipeline
            empty = {}
        else:
            empty = None
        ...
        for proto, configs in (by_protocol or {}).items():
            self._write_subscription(f"sub_{proto}.txt", configs)

        # если хранилище пусто, создаём хотя бы один файл, чтобы был коммит
        if empty:
            self._write_subscription("sub_all_checked.txt", [])
